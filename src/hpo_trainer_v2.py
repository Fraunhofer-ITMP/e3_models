...
def save_model(model, exp_name):
    ...
    model_path = f"../data/models/{exp_name}.pkl"
...
def save_params(params, exp_name):
    ...
    with open(f"../data/models/{exp_name}_params.json", "w") as f:
...
def save_trial(trial, model, study_name):
    ...
    trial_path = f"../data/experiments/{study_name}"
...
def objective_xgboost(trial, study_name, X_train, y_train):
    ...
    label_to_idx = {
        ...
        #"CRBN":0 , "VHL": 1, "IAP":2, "XIAP" :3, "Other": 4
...
def report(y_test, y_pred, model, exp_name: str) -> pd.DataFrame:
    ...
    if len(label_classes) > 2: 
        ...
        average_fpr, average_tpr, _ = roc_curve(
            y_test_binarize, y_pred_binarize
        )
        print(y_test)

        y_test_float = y_test.array
        y_pred_float = y_pred.array

        np.place(y_test_float,y_test_float== "YES",0.0)
        np.place(y_test_float,y_test_float== "NO",1.0)

        np.place(y_pred_float, y_pred_float == "YES", 0.0)
        np.place(y_pred_float, y_pred_float == "NO", 1.0)
        roc_auc['result'] = roc_auc_score(y_test_float, y_pred_float)
        #print(fpr,tpr)
