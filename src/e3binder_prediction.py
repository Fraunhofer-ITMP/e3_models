...
from hpo_trainer_v2 import objective_rf, report, SEED, finalize_model, objective_xgboost
...
ecfp4_dataframe.to_csv("../data/fingerprints/ecfp4_test.csv",index_label=False)
...
xgboost_model = pickle.load(open(f"../data/e3binder_predictor_model/models/ecfp4_xgboost.pickle.dat", "rb"))

test_data = pd.read_csv(f"../data/fingerprints/ecfp4_test.csv")
...
label_to_idx = {0: "YES", 1: "NO"}

# Label mapping to integers
df_predicted['Predicted'] = df_predicted['Predicted'].map(label_to_idx)

colnames = 'YES NO Predicted'.split(" ")

df_predicted.columns = colnames

df_predicted['SMILES'] = list(df["SMILES"])

os.makedirs("../data/e3binder_predictor_model/output", exist_ok=True)

df_predicted.to_csv("../data/e3binder_predictor_model/output/e3_binder_prediction.csv",index = False)

print('Process completed. Please look at the results in e3binder_predictor_model/output folder')